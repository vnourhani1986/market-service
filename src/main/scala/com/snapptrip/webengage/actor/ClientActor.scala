...
class ClientActor(
  ...
  lazy val userActorRef: ActorRef = context.actorOf(FromConfig.props(UserActor(dbRouter)), s"user-router")
  lazy val eventActorRef: ActorRef = context.actorOf(FromConfig.props(EventActor(dbRouter)), s"event-router")
